...
public interface LetterApi {
    ...
    ResponseEntity<?> create(@RequestBody LetterRequest letterRequest);

    @Operation(summary = "편지 단건 조회")
    ResponseEntity<LetterResponse> findById(@PathVariable("letterId") Long letterId);

    @Operation(summary = "편지 전체 조회")
    ResponseEntity<List<LetterSimpleResponse>> findAllByMember(@PathVariable("memberId") Long memberId);

    @Operation(summary = "편지 삭제")
    ResponseEntity<?> delete(@LoginMember Long memberId, @PathVariable("letterId") Long letterId);
}
